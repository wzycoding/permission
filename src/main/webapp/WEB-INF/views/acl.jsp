...
<script type="text/javascript">
    $(function () {
        var aclModuleMap = {}; //存储map格式权限模块
        var aclModuleList = [];// 权限模块
        var aclMap = {};//存储权限信息
        var optionStr = "";
        ...
        function recursiveRenderAclModule(aclModuleList) {
            if (aclModuleList && aclModuleList.length > 0) {
                $(aclModuleList).each(function (i, aclModule) {
                    ...
                    if (aclModule && aclModule.aclModuleList.length > 0) {
                        var rendered = Mustache.render(aclModuleListTemplate, {
        ...
        function handleAclModuleSelected(aclModuleId) {
            if (lastClickAclModuleId != -1) {
                console.log("移除样式")
                var lastDept = $("#aclModule_" + lastClickAclModuleId + " .dd2-content:first");
                    ...
                    buttons: {
                        "更新": function (e) {
                            ...
                            updateAcl(false, function (data) {
                                ...
                                showMessage("更新权限点", data.msg, false)
                            })
                        },
                        "取消": function () {
                            $("#dialog-acl-form").dialog("close");
        ...
        function updateAcl(isCreated, successCallback, failCallback) {
            $.ajax({
                url: isCreated? '/sys/acl/save.json' : '/sys/acl/update.json',
