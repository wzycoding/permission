...
<script type="text/javascript">
    $(function() {
        var roleMap = {};
        var lastRoleId = -1;
        //一个角色与用户，一个角色于权限，使用tab进行维护，通过selectFirst来进行区别。
        ...
        function renderRoleTree(aclModuleList) {
            ...
            //后将nodeMap放入zTreeObj,是因为可能会改动，保存的权限模块信息
        ...
        function recursivePrepareTreeData(aclModuleList) {
            ...
            if (aclModuleList && aclModuleList.length > 0 ) {
                $(aclModuleList).each(function (i, aclModule) {
                    //是否需要勾选 是否包含权限点，如果下面有权限点被勾选，则父节点被勾选
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result.ret) {
                        ...
                        showMessage("保存角色与权限点关系", result.msg, false)
                    }
        ...
        $(".saveRoleUser").click(function (e) {
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result.ret) {
                        showMessage("保存角色与用户关系", "操作成功", true)
                    } else {
                        showMessage("保存角色与用户关系", result.msg, false)
                    }
        ...
        function updateRole(isCreate, successFallback, failFallback) {
            $.ajax({
        ...
        $("#roleTab a[data-toggle='tab']").on("shown.bs.tab", function(e) {
            if (lastRoleId == -1) {
                showMessage("加载角色关系", "请现在左侧选择操作的角色", false);
                return
            }
