    ...
    public void recover(int id) {
        ...
        switch (sysLog.getType()) {
            ...
            case LogType.TYPE_ROLE:
                ...
                Preconditions.checkNotNull(beforeRole, "待还原的用户已经不存在了");
                if (StringUtils.isBlank(sysLog.getNewValue() ) || StringUtils.isBlank(sysLog.getOldValue())) {
                    throw new ParamException("新增和删除操作不做还原");
                }
            ...
            case LogType.TYPE_ROLE_ACL:
                ...
                sysRoleAclService.changeRoleAcls(sysLog.getTargetId(), JsonMapper.string2Object(sysLog.getOldValue(), new TypeReference<List<Integer>>() {}));
                break;
            case LogType.TYPE_ROLE_USER:
                SysRole aclUser = sysRoleMapper.selectByPrimaryKey(sysLog.getTargetId());
                Preconditions.checkNotNull(aclUser, "角色已经不存在了");
                sysRoleUserService.changeRoleUsers(sysLog.getTargetId(), JsonMapper.string2Object(sysLog.getOldValue(), new TypeReference<List<Integer>>() {}));
                break;
            default:
                break;
        }
